...
console.log('1. Baseline prompt contains the current humanized sections')
{
    ...
    assertContains(prompt, '2 responders: most normal turns — one main reply, one riff or reaction that adds something', 'responder restraint rule')
    ...
    assertContains(prompt, 'current profile name is authoritative', 'current profile name wins over stale context')
